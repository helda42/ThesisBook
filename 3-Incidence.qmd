...
Data on NTS blood and CSF cultures was available from `{r} print(stdate)` to `{r} print(endate)`. During this period there were `{r} print(prettyNum(as.integer(sum_stats$total), big.mark=","))` cases of iNTS, including `{r} print(prettyNum(sum_stats$salmonella_typhimurium,big.mark=","))` S.typhimurium, `{r} print(prettyNum(sum_stats$salmonella_enteritidis,big.mark=","))` S.enteritidis, and `{r} sum_stats$salmonella_species` S.species (@tbl-sumtab). Data for age was missing for a total of `{r} print(tot_missAge)` NTS isolates; the distribution of age categories and missing ages by year are shown in @fig-NoPeriod. Data was available on total number of blood cultures collected from 1998-01-01 to 2022-05-01; with a total of `{r} print(prettyNum(totBC))` [@tbl-sumtab]. The total Blantyre population was `{r} print(prettyNum(as.integer(startpop),big.mark=","))` in `{r} print(startyr)` and `{r} print(prettyNum(as.integer(endpop),big.mark=","))` in `{r} print(endyr)`. During this period, the minimum incidence of blood culture confirmed iNTS ranged from `{r} print(lowinc)` in `{r} print(lowyr$Year)` to `{r} print(hiinc)` in `{r} print(hiyr$Year)` per 100,000 population (@tbl-sumtab, @fig-NoPeriod).
...
#| fig-width: !expr size_list$fig_width
#| label: fig-NoPeriod
#| fig-cap: Number of Non-Typhoidal Salmonella blood culture isolates by time period and by serovar at Queen Elizabeth Central Hospital (QECH), 1996-2024; A
#| 
p96_15 <- age_gp %>% 
  filter(Year<2016) %>% 
...
```{r echo=FALSE, results='hide', warning=FALSE, message=FALSE, fig.align = "left", fig.height=7, fig.width=5.5}
#| label: fig-HistMiss
#| fig-cap: Count of Non-Typhoidal Salmonella blood culture isolates by age group, Queen Elizabeth Central Hospital (QECH), 1996-2024
...
cowplot::plot_grid(pAgeU5, incAge, nrow=2)
