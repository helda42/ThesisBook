...
$$ log(cases) = ~ \alpha + \beta_{2}*bspline(age, df=8) + log(population) $$ Data smoothing using B-splines was implemented for the
...
startpop <- min(inc_tab_long2$`Population Census`, na.rm=T)
endpop <- max(inc_tab_long2$`Population Census`, na.rm=T)
...
Data on NTS blood and CSF cultures was available from `{r} print(stdate)` to `{r} print(endate)`. During this period there were `{r} as.integer(sum_stats$total)` cases of iNTS, including `{r} sum_stats$salmonella_typhimurium` S.typhimurium, `{r} sum_stats$salmonella_enteritidis` S.enteritidis, and `{r} sum_stats$salmonella_species` S.species (@tbl-flextable.). Data was available on total number of blood cultures collected from 1998-01-01 to 2022-05-01. The total Blantyre population was `{r} print(prettyNum(as.integer(startpop),big.mark=","))` in `{r} print(startyr)` and `{r} print(prettyNum(as.integer(endpop),big.mark=","))` in `{r} print(endyr)`. During this period, the minimum incidence of blood culture confirmed iNTS ranged from `{r} print(lowinc)` in `{r} print(lowyr$Year)` to `{r} print(hiinc)` in `{r} print(hiyr$Year)` per 100,000 population (@tbl-flextable., @fig-1.).
...
#| fig-width: !expr size_list$fig_width
#| label: fig-1
#| fig-cap: Number of Non-Typhoidal Salmonella blood culture isolates from Queen Elizabeth Central Hospital (QECH), 1996-2024
#| echo: false
#| 
p96_15 <- age_gp %>% 
  filter(Year<2016) %>% 
...
#| fig-width: !expr size_list$fig_width
#| label: fig-2
#| fig-cap: Minimum Incidence of Non-Typhoidal Salmonella blood culture isolates, Queen Elizabeth Central Hospital (QECH), 1996-2024
#| echo: false
#| 
ggplot(inc_tab_long2, aes(x=Year, y=`Minimum Incidence`))+
  geom_point()+
  geom_line()+
  # ggtitle("Minimum Incidence iNTS (BC) by year, QECH, Blanytre")+
...
#| fig-width: !expr size_list$fig_width
#| label: fig-4
#| fig-cap: Minimum Incidence of Non-Typhoidal Salmonella blood culture isolates in children aged 0-60 months, Queen Elizabeth Central Hospital (QECH), 1996-2024
#| echo: false
...
iNTS_U5_1996_2024_sum <- iNTS_all_U5 %>% 
  ...
  ungroup() %>% 
  mutate(pop = 2401,
         risk = count/pop*10000)
  
iNTS_U5_1996_2024_sumYr <- iNTS_all_U5 %>% 
  mutate(agemth_rnd = round(Age_months, digits=0)) %>% 
  group_by(agemth_rnd, YearBand3) %>% 
  summarise(midage = mean(Age_months),
            count = n())%>% 
  ungroup() %>% 
  left_join(BTpop_u5) %>% 
  mutate(risk = count/pop*100000)

iNTS_U5_1996_2024_sumYr2 <- iNTS_all_U5 %>% 
  mutate(agemth_rnd = round(Age_months, digits=0)) %>% 
  group_by(agemth_rnd, YearBand) %>% 
  summarise(midage = mean(Age_months),
            count = n())%>% 
  ungroup() %>% 
  left_join(BTpop_u52) %>% 
  mutate(risk = count/(pop/12)*100000,
...
risk_predict <- function(df, name ){ #Switch, grp
  spline_risk <-glm.nb(count ~ bs((midage), df=8)+ offset(log(pop)), df) 
  ...
  mutate(case_pred=exp(pred_obj$fit),
         ...
         sd=(risk_high-risk_low/3.92)*sqrt(count),
...
#| fig-width: !expr size_list$fig_width
#| label: fig-5
#| fig-cap: Minimum Incidence of Non-Typhoidal Salmonella blood culture isolates in children aged 0-60 months by organism, Queen Elizabeth Central Hospital (QECH), 1996-2024
#| echo: false
#| 
...
#| fig-width: !expr size_list$fig_width
#| label: fig-9
#| fig-cap: Multidrug resistant (MDR) (resistant to ampicillin, cotrimoxazole, and chloramphenicol) trends over time by agegroup at QUECH, Blantyre (2010 - 2024)
...
There are differences between the methodologies used between sites in Malawi, Burkina Faso and Kenya on these field sites (explored in Carey et al, Clin Infect Dis, 2020), which make it difficult to directly compare these data, and adjusting crude data to give an adjusted incidence adds additional variability and has not yet been undertaken for the Malawi estimates. However, the most directly comparable data sets, on crude unadjusted incidence (see below) in the relevant age-bands, suggest that Malawi could have the highest age-linked incidences of iNTS. Country Crude incidence, iNTS cases per 100,000 PYO Adjusted incidence iNTS cases per 100,000 PYO.
